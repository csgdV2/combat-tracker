...
public class ModMenuIntegration implements ModMenuApi {
    ...
    private static void buildGeneral(ConfigBuilder builder, ConfigEntryBuilder eb, CtConfig config) {
        ...
        general.addEntry(eb.startBooleanToggle(Component.literal("Show HUD"), config.hudEnabled)
                ...
                .setTooltip(Component.literal("Master switch for the overlay. Its optional keybind is in Controls > Key Binds > Combat Tracker."))
        ...
        general.addEntry(eb.startSelector(Component.literal("Layout"), HUD_LAYOUTS, layoutName(config.hudLayout))
                .setDefaultValue(HUD_LAYOUTS[2])
                .setNameProvider(Component::literal)
                .setTooltip(Component.literal("Full shows every stat. Compact condenses them. Recording only shows the recording timer."))
                .setSaveConsumer(v -> config.hudLayout = layoutIndex(v))
                .build());
        ...
        general.addEntry(eb.startSelector(Component.literal("Theme"),
                ...
                .setDefaultValue(HudRenderer.THEME_NAMES[1])
                .setNameProvider(Component::literal)
    ...
    private static String themeName(int index) {
        if (index < 0 || index >= HudRenderer.THEME_NAMES.length) {
            return HudRenderer.THEME_NAMES[1];
        }
    ...
    private static final String[] HUD_LAYOUTS = {"Full", "Compact", "Recording only"};

    private static String layoutName(int index) {
        return index >= 0 && index < HUD_LAYOUTS.length ? HUD_LAYOUTS[index] : HUD_LAYOUTS[2];
    }

    private static int layoutIndex(String name) {
        for (int i = 0; i < HUD_LAYOUTS.length; i++) {
            if (HUD_LAYOUTS[i].equals(name)) {
                return i;
            }
        }
        return 2;
    ...
    private static void buildRecording(ConfigBuilder builder, ConfigEntryBuilder eb) {
        ...
        recording.addEntry(new ButtonEntry(Component.literal("Open latest report"), b -> {
            if (!SessionRecorder.get().openLatestReport()) {
                b.setMessage(Component.literal("No local report yet").withStyle(ChatFormatting.RED));
            }
        }));
    ...
    private static Component recordLabel() {
        return SessionRecorder.get().isPlayerRecording()
